...
Pick a different tag identifier or instance key without the `compose:` prefix. The reserved prefix exists so the compose engine can own its namespace without colliding with hand-managed entities. If you genuinely need to create a `compose:`-prefixed name, do it through `rimsky compose up` (the compose engine), not through the hand-tag or `POST /instances` paths.
