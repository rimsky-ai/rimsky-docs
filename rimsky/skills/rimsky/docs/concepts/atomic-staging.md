...
- 2026-06-06 — The **producer-side staging-schema lifecycle for the Postgres SQL substrate now ships** (superseding the 2026-05-19 "not yet shipped" status), per spec:2026-06-06-comprehensive-gap-closure. For a staged_async claim whose selector names a schema, the postgres store reserves a per-claim staging schema at Open and returns it as the claim address (the executor writes into it); the claim scope stays the canonical selector so claim-scope byte-equality / conflict detection is unaffected. Commit performs the atomic schema swap inside one store-side transaction (drop the canonical, rename staging into its place), Abandon discards the staging schema, and Release reaps any residual staging from an interrupted claim. The swap drops the canonical without cascading across schemas — a no-surprise-data-loss property: a populated or externally-depended-upon canonical is refused rather than silently clobbered, surfacing the `pg/swap_failed` error class (giving that declared class a real emit site). The lifecycle engages only for schema-shaped selectors; opaque (path-shaped) scope-bytes claims keep the verbatim selector-echo at Open and no-op terminals. Substrate-atomicity table unchanged ("Postgres schema swap: atomic via transaction" already named this envelope).
