...
For the `validation` mix-in, the role list rimsky honors is the **live** `validation_supported_roles` from the peer's primary-protocol `Capabilities` handshake, not anything in `rimsky.yml`: when a claim-producer peer advertises `validation`, rimsky runs a fresh `ClaimProducer.Capabilities` RPC at startup to learn the roles (one extra RPC per validation-mix-in peer; a failure fails startup). Executor and publisher peers advertising `validation` are dialed, but their supported-roles list is currently treated as **empty** — their primary-protocol capability surfaces are not yet plumbed into the validation registry. <!-- @source: lib/control/config/publishers.go -->
