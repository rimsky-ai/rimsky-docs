...
- 2026-05-29 — Per `spec:2026-05-29-console-upstream-auth-audit-and-fixes`: clarified that the `partition_request` is **resolved through substitution at acquisition** (binding the triggering message's payload), not passed verbatim to the split — completing the half-built backfill-override path (see `concept:backfill`, `concept:message`). Also fixed a pre-existing doc bug: the illustrative directive used `| default: <x>`, which is not the engine's grammar — the fallback is `<directive> | <literal>` with no `default:` keyword.
