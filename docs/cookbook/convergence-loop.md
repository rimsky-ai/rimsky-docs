...
`http-node` executor. There is an important truth to state up front: on
`deploy/`, `http-node` runs in stub mode (`RIMSKY_EXECUTOR_STUB_MODE=1`)
and **always reports `changed: true`**. So the `when: payload.changed`
self-edge fires on *every* iteration and never finds the `changed: false`
that would let the loop converge. What you can demonstrate end-to-end on
`deploy/` is therefore the loop *shape*: the node re-fires itself frame
after frame, each iteration a clean `terminal/success`. Because each
re-fire is a success (not an error-policy retry), the loop is **not**
bounded by the `retry_loop_no_progress` cap — on the stub it runs until
you terminate the instance. Natural converge-then-settle needs a *real*
executor whose `changed` flips to `false` once the work stops moving — see
the note at the end.
...
On `deploy/` the stub always reports `changed: true`, so the gate matches
...
The recipe's lesson on `deploy/` is the loop *shape*: a loop is one
